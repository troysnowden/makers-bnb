...
describe AccommodationAccess do
  ...
  describe '#all_owned_by_user' do
    it 'should return an empty array when user has no created accommodations' do
      expect(AccommodationAccess.all_owned_by_user(new_user.user_id).length).to eq(0)
    end

    it 'should return an array with one accommodation object when user has one created accommodation' do
      AccommodationAccess.create(new_user.user_id, "My accommodation", "Brief description", 30)
      
      accommodations = AccommodationAccess.all_owned_by_user(new_user.user_id)
    ...
    it 'should not return any accommodations not owned by user' do
      ...
      AccommodationAccess.create(not_our_user.user_id, "Not My accommodation", "Brief description", 30)
  ...
  describe '#all_available_within_max_price_on_date' do
    it 'should return only accommodation below the given max price' do
      # Accommodation that should not be returned by this method
      AccommodationAccess.create(not_our_user.user_id, "Not valid accommodation", "Brief description", 70)

      # Accommodation that should be returned by this method
      AccommodationAccess.create(new_user.user_id, "Valid accommodation", "Brief description", 60)
    ...
    it 'should return only accommodation available on date given' do
      ...
      new_accom = AccommodationAccess.create(not_our_user.user_id, "Unavailable accommodation", "Brief description", 30)
      ...
      # Accommodation that should be returned by this method
      AccommodationAccess.create(new_user.user_id, "Valid accommodation", "Brief description", 30)
  ...
  describe '#create' do
    it 'should add a new accommodation to the accommodations table' do
      ...
      created_accommodation = AccommodationAccess.create(new_user.user_id, "My accommodation", "Brief description", 30)
